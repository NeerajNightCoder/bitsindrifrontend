...
import PostCard, { Profile_Min } from "../components/Community/PostCard";

interface Profile_Max {
  id: string;
  name: string;
  email: string;
...
const Community = () => {
  ...
  const [profiles, setProfiles] = useState<Profile_Max[]>([]);
...
if (error) {
  console.error("Error fetching posts:", error);
    ...
    //   else setPosts(data || []);
    else setPosts(data.map(item=>({...item,profiles:Array.isArray(item.profiles)?item.profiles[0]:item.profiles})));
      console.log(data)
    };
    fetchPosts();
  }, [supabase]);
    ...
    // Filter function
    const filterProfiles = (profile: Profile_Max) => {
        return (
  ...
  return (
    <div className="w-1/2 mx-auto p-4 overflow-scroll max-h-screen">
...
const ProfileCard = ({ profile }: { profile: Profile_Max }) => (
  <div className="bg-white shadow-lg rounded-lg p-4">
    <div className="flex flex-col items-center">
      <Image width={100} height={100} src={profile.profileimg || "https://cdn-icons-png.flaticon.com/512/149/149071.png"} alt={profile.name} className="w-24 h-24 rounded-full object-cover border-2 border-gray-300" />
